/mob/living/simple_animal/space_worm
	...
	a_intent = "harm" //so they don't get puxed around
	...
	proc/AttemptToEat(var/atom/target)
		if(istype(target,/turf/simulated/wall))
			if((!istype(target,/turf/simulated/wall/r_wall) && eatingDuration >= 100) || eatingDuration >= 200) //need 20 ticks to eat an rwall, 10 for a regular one
				...
				new /obj/item/stack/xeet/metal(src, flatPlasmaValue)
				return 1
		...
		for(var/atom/movable/stomachContent in contents)
			if(prob(digestionProbability))
				if(istype(stomachContent,/obj/item/stack)) //converts to plasma, keeping the stack value
					if(!istype(stomachContent,/obj/item/stack/xeet/mineral/plasma))
						var/obj/item/stack/oldStack = stomachContent
						new /obj/item/stack/xeet/mineral/plasma(src, oldStack.amount)
						del(oldStack)
						continue
				else if(istype(stomachContent,/obj/item)) //converts to plasma, keeping the w_class
					var/obj/item/oldItem = stomachContent
					new /obj/item/stack/xeet/mineral/plasma(src, oldItem.w_class)
					del(oldItem)
					continue
				else
					new /obj/item/stack/xeet/mineral/plasma(src, flatPlasmaValue) //just flat amount
					del(stomachContent)
					continue
