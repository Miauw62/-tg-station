...
/mob/living/carbon/human/proc/handle_breath(datum/gas_mixture/breath) //This proc was rather terrible, and now it's less terrible. ~Miauw
	if(status_flags & GODMODE)
		return 1

	var/has_inaprovaline = reagents.has_reagent("inaprovaline")

	if(suiciding && !has_inaprovaline)
		adjustOxyLoss(HUMAN_MAX_OXYLOSS + 2) //Suiciding people die a little faster
		failed_last_breath = 1
		oxygen_alert = max(oxygen_alert, 1)
		return 0

	if(health <= config.health_threshold_crit && !has_inaprovaline) //If we're in crit without any inaprovaline...
		adjustOxyLoss(HUMAN_CRIT_MAX_OXYLOSS)
		failed_last_breath = 1

	if((!breath || !breath.total_moles()) && !has_inaprovaline) //A bit hacky, but else there are zero division errors EVERYWHERE.
...
//Oxygen checks.
	if(O2_pp < safe_oxygen_min)
		...
		if(O2_pp < safe_oxygen_min) 			// Too little oxygen
			if(prob(20))
				emote("gasp")
			if(O2_pp > 0)
				var/oxygen_deficiency_ratio = safe_oxygen_min / O2_pp
				if(!has_inaprovaline)
					adjustOxyLoss(min(5 * oxygen_deficiency_ratio, HUMAN_MAX_OXYLOSS)) // Don't fuck them up too fast (space only does HUMAN_MAX_OXYLOSS after all!)
				failed_last_breath = 1
				oxygen_used = breath.oxygen * oxygen_deficiency_ratio / 6
			else
				if(!has_inaprovaline)
					adjustOxyLoss(HUMAN_MAX_OXYLOSS)
				failed_last_breath = 1
	...
	else // Enough air to breathe. FOR NOW :)
	...
	if(CO2_pp > safe_co2_max)
		if(!co2overloadtime) // If it's the first breath with too much CO2 in it, lets start a counter, then have them pass out after 12s or so.
			...
			if(world.time - co2overloadtime > 300) // They've been in here 30s now, lets start to kill them for their own good!
				adjustOxyLoss(8)
		if(dieing_of_co2 && prob(20)) // Lets give them some chance to know somethings not right though I guess.
			emote("cough")
...
//Trace gasses (aka nitrous oxide because nobody ever added any other trace gasses) checks.
	...
	if(stat == DEAD)	//DEAD. BROWN BREAD. SWIMMING WITH THE SPESS CARP
		...
		if(dizziness)
			...
			if(C)
				...
				src = oldsrc //FOR THE LOVE OF GOD WHY ;_;
