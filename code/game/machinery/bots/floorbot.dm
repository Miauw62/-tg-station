...
/obj/machinery/bot/floorbot/process()
	...
	if(src.amount <= 0 && ((src.target == null) || !src.target))
		...
		if(src.target == null || !src.target)
			if(src.maketiles)
				if(src.target == null || !src.target)
					for(var/obj/item/stack/xeet/metal/M in view(7, src))
	...
	if(src.loc == src.target || src.loc == src.target.loc)
		...
		else if(istype(src.target, /obj/item/stack/xeet/metal))
...
/obj/machinery/bot/floorbot/proc/maketile(var/obj/item/stack/xeet/metal/M)
	if(!istype(M, /obj/item/stack/xeet/metal))
		return
	if(M.amount > 1)
		return
